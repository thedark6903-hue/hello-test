...
function Blocked([string]$p) {
  $r=Rel $p
  if ($Protected -contains $r) { return $true }
  foreach($x in $ProtectedPrefixes) {
    if ($r -eq $x -or $r.StartsWith("$x/")) { return $true }
  }
  return $false
}
function Allowed([string]$p) {
  $r=Rel $p
  return ($r -eq "src" -or $r.StartsWith("src/") -or
          $r -eq "public" -or $r.StartsWith("public/"))
}
...
try {
  ...
  Copy-Item $ProjectRoot $Work -Recurse -Force

  Write-Host "Applying ONLY allowed source files..." -ForegroundColor Cyan
  Get-ChildItem $Source -Recurse -File -Force | % {
    $r=Rel $_.FullName.Substring($Source.Length)
    if (-not (Allowed $r)) { return }
    if (Blocked $r) {
      Write-Host "SKIP protected: $r" -ForegroundColor Yellow
      return
    }
    $dest=Join-Path $Work $r
  ...
  Set-Location $ProjectRoot
  Write-Host ""
  Write-Host "DRY RUN SUCCESS" -ForegroundColor Green
  ...
  Write-Host ""
  Write-Host "Protected: version files, package files, GitHub workflow, Android, configs." -ForegroundColor Yellow
}
catch {
  Write-Host "DRY RUN FAILED: $($_.Exception.Message)" -ForegroundColor Red
  Write-Host "REAL PROJECT WAS NOT MODIFIED." -ForegroundColor Yellow
  exit 1
}
finally {
  Set-Location $ProjectRoot
